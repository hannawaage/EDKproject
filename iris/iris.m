...
W0           = 0.01*ones(C,D+1);                         % Initial weight matrix
sigmoid      = @(x) 1./(1+exp(-x));                      % Sigmoid function
discriminant = @(xk,W) sigmoid(W*xk);                    % Discriminant function
...
%% Training
%[W,FVAL,EXITFLAG,OUTPUT,GRAD] = fminunc(@(w)MSE(X,T,w,discriminant), W0);
[W, niterations] = gradient_descent(@(W)MSE_grad(X,T,W,discriminant), W0, alpha, maxiter);


%% Testing
Xtest       = [xtest'; ones(1, size(xtest,1))];               % Test cases
Ttest       = [repelem(1,20), repelem(2,20), repelem(3,20)];  % Solution
[~,classes] = max(W*Xtest);                                   % Predicted classes
